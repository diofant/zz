...
zz_to_bytes(const zz_t *u, size_t length, bool is_signed, uint8_t **buffer)
{
    ...
    if (is_negative) {
        ...
        if (zz_resize(8*(int64_t)length/ZZ_LIMB_T_BITS + 1, &tmp)) {
            return ZZ_MEM; /* LCOV_EXCL_LINE */
        }
        if (tmp.size < u->size) {
            goto overflow;
...
zz_addsub(const zz_t *u, const zz_t *v, bool subtract, zz_t *w)
{
    ...
    if (zz_resize((int64_t)u_size + same_sign, w)) {
        return ZZ_MEM; /* LCOV_EXCL_LINE */
    }
    w->negative = negu;
...
zz_mul(const zz_t *u, const zz_t *v, zz_t *w)
{
    ...
    if (zz_resize((int64_t)u->size + v->size, w) || TMP_OVERFLOW) {
        return ZZ_MEM; /* LCOV_EXCL_LINE */
    }
...
zz_mul_sl(const zz_t *u, zz_slimb_t v, zz_t *w)
{
    ...
    if (zz_resize(u->size + 1LL, w) || TMP_OVERFLOW) {
        return ZZ_MEM; /* LCOV_EXCL_LINE */
    }
...
zz_div(const zz_t *u, const zz_t *v, zz_t *q, zz_t *r)
{
    ...
    if (!u->size) {
        ...
        if (zz_resize((int64_t)u_size - v->size + 1 + q_negative, q)
...
zz_quo_2exp(const zz_t *u, zz_limb_t shift, zz_t *v)
{
    ...
    if (zz_resize((int64_t)size + extra, v)) {
        return ZZ_MEM; /* LCOV_EXCL_LINE */
    }
    v->negative = u->negative;
    if (shift) {
...
zz_gcdext(const zz_t *u, const zz_t *v, zz_t *g, zz_t *s, zz_t *t)
{
    ...
    if (zz_init(o1) || zz_init(o2)
        ...
        || zz_resize(v->size, tmp_g) || zz_resize(v->size + 1LL, tmp_s)
        || TMP_OVERFLOW)
    {
        goto clear; /* LCOV_EXCL_LINE */
    }
...
zz_sqrtrem(const zz_t *u, zz_t *v, zz_t *w)
{
    ...
    if (zz_resize((u->size + 1LL)/2, v) == ZZ_MEM || TMP_OVERFLOW) {
        return ZZ_MEM; /* LCOV_EXCL_LINE */
    }
    if (w) {
        w->negative = false;
